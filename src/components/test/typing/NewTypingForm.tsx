...
import { TypingQuesion } from "@/utils/classes";
...
export default function NewTypingForm() {
  const [formState, setFormState] = useState<TypingQuesion>(
    new TypingQuesion(),
  );
  const [title, setTitle] = useState<string>("");
  const [subject, setSubject] = useState<string>("");
  const [questions, setQuestions] = useState<Array<TypingQuesion>>([]);
  ...
  const submitHandler: React.FormEventHandler<HTMLFormElement> = (event) => {
    ...
    setFormState(new TypingQuesion());
  };
  ...
  function createTest() {
    ...
    const p = axios.post("/api/typing/new", {
      title,
      subject,
      releaseDate,
      withholdDate,
